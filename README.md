...
After activation, environment variables define the working paths:

```text
JANGO_SOURCE=/path/to/jango_repo
JANGO_WORK_ROOT=/path/to/user_root/work
JANGO_OUTPUTS_ROOT=/path/to/user_root/outputs
JANGO_DATA_ROOT=/path/to/user_root/data
JANGO_RAW_TARBALLS=/path/to/jango_repo/data/tarballs
FOLDING_BACKEND=esmfold2
MSA_CACHE_ROOT=/path/to/jango_repo/data/msa
...
The GitHub repository contains code, tests, documentation, and small report assets. Large generated datasets should remain on external storage or be published as separate artifacts.
...
Exposed or omitted from Git:

* Full fold outputs, relaxed PDBs, raw tarballs, MSA caches, Slurm logs, and run-local `work/` or `tmp/` folders.
* Large curated data under `data/`; this remains stored externally unless separately packaged.

## Repository And Data Layout

The Git source repository stays mostly source-only:
...
`data/msa` is the runtime MSA cache Jango points to by default. `data/msa_cache` is maintained for backwards compatibility. Put structural meaning in the run ID, backend name, redesign mode, and table metadata.
...
`validation/` are the primary report-facing packages. Use the package
manifests to trace figures back to their canonical supporting tables.
...
Report-ready hotspot packages:

```text
jango/data/outputs/evaluation/decoys
jango/data/outputs/evaluation/validation
...
```text
local_copy/data/outputs/evaluation/decoys
local_copy/data/outputs/evaluation/validation
...
Only folding requires GPU access. ProteinMPNN, MSA resolution, sequence validation, fold QC, native analysis, Rosetta-side metrics, DockQ, and figure generation run on standard CPU nodes.
...
* Cluster logs and resolved configs only when needed for reproducibility/debugging.
...
When debugging path issues, audit generated manifests and tables for stale paths or old validation run IDs before executing long jobs.
...
* **Path lineage must remain strict.** Stale absolute paths can cause downstream failures. Every manifest consumed by a later stage should be auditable and canonical.
