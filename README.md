...
Jango is the DELPHI nanobody decoy-generation and evaluation toolkit. It combines the preserved `nbia` scientific engine with Jango orchestration for input preparation, antigen redesign, folding, grafting, Rosetta relax/scoring, DockQ/TM-score comparisons, and compact evaluation figures/tables.

The shared <SLURM_NODE> source repository is:

```bash
<JANGO_REPO>
```

Normal generated data should not be written into the source tree unless it is a curated shared dataset/output package. User runs belong under `<JANGO_USER_ROOT>`; validated shared data belongs under `<JANGO_REPO>/data`.
